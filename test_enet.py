...
#Directories
flags.DEFINE_string('dataset_dir', './IronData_Camvid_train_val', 'The dataset directory to find the train, validation and test images.')
#flags.DEFINE_string('dataset_dir', './test', 'The dataset directory to find the train, validation and test images.')
flags.DEFINE_string('checkpoint_dir', './log/train_original_MFB_combined_data', 'The checkpoint directory to restore your mode.l')
flags.DEFINE_string('logdir', './log/train_original_MFB_combined_data', 'The log directory for event files created during test evaluation.')
flags.DEFINE_boolean('save_images', True, 'If True, saves 10 images to your logdir for visualization.')

#Evaluation information
flags.DEFINE_integer('num_classes', 4, 'The number of classes to predict.')
flags.DEFINE_integer('batch_size', 20, 'The batch_size for evaluation.')
...
#Dataset directories
image_files = sorted([os.path.join(dataset_dir, 'val', file) for file in os.listdir(dataset_dir + "/val") if file.endswith('.jpg')])
annotation_files = sorted([os.path.join(dataset_dir, "valannot", file) for file in os.listdir(dataset_dir + "/valannot") if file.endswith('.png')])
